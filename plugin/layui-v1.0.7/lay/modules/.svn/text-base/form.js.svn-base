...
layui.define('layer', function(exports){
  ...
  //设置表单
  Form.prototype.getFormJson = function(formselect){
    var json = [];

    jQuery(formselect).each(function(){
      var single = {};
      // 单行文本框
      jQuery(this).find(':text').each(function(){
        if(this.name) {//不需要自动生成的text
          jQuery.trim(this.value) && (single[this.name] = jQuery.trim(this.value));
        }
      ...
      // 单选按钮
      jQuery(this).find('input:radio:checked').each(function(){

        jQuery.trim(this.value) && (single[this.name] = this.value);
  ...
  //设置表单元素
  Form.prototype.setFormData = function(data,formselect){
    for(var i in data){
      $(formselect+' [name='+i+']').each(function(index, element) {
        var that=$(this);
        if(that.is('input[type=text]') || that.is('input[type=passWord]') || that.is('input[type=hidden]') || that.is('textarea')){
          that.val(data[i])
        }else if(that.is('input[type=checkbox]') || that.is('input[type=radio]')){
          var D=(data[i]+'').split(',');
          $(D).each(function(index, element) {
            that.attr('checked',that.val() == element);
            if(that.val() == element) return false;
          });
        }else if(that.is('select')){
          $('option[value='+data[i]+']',this).attr('selected',true).siblings().attr('selected',false);
        }
      });
    }
  }
  //验证form
  Form.prototype.verifyForm = function(formselect){
    var  verify = form.config.verify
        ,DANGER = 'layui-form-danger', field = {} ,elem = $(formselect),stop = false
        ,verifyElem = elem.find('*[lay-verify]') //获取需要校验的元素
        ,formElem = $(formselect) //获取当前所在的form元素，如果存在的话
